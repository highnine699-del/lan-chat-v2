...
def register_admin_handlers(sio):
    ...
    async def handle_kick(sid, data):
        ...
        sio.leave_room(target_sid, room_id)
    ...
    async def handle_server_kick(sid, data):
        ...
        target_disp = str(data.get('target', ''))
        reason = str(data.get('reason', 'Kicked by server admin.'))[:200]
        target_sid = sid_map.get(target_disp)
    ...
    async def handle_server_shadow_mute(sid, data):
        ...
        target_disp = str(data.get('target', ''))
        duration_s = min(int(data.get('duration', 300)), 86400)
        target_sid = sid_map.get(target_disp)
        ...
        await sio.emit('system_message',
             system_msg(f'Moderation action applied.'),
             to=sid)
